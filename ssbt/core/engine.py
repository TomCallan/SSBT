...
class Engine:
    ...
    def _run_fast_bar(self) -> None:
        """Fast path: single-symbol bar data, pre-allocated Bar reuse."""
        ...
        # Pre-allocate one Bar object — mutate fields per bar, no allocation
        bar = Bar(timestamp=0, symbol=symbol, open=0.0, high=0.0, low=0.0, close=0.0, volume=0.0)

        for i in range(n):
            self._event_count += 1

            # Mutate in place — zero allocation
            bar.timestamp = int(ts[i])
            bar.open = float(o[i])
            bar.high = float(h[i])
            bar.low = float(l[i])
            bar.close = float(c[i])
            bar.volume = float(v[i])
    ...
    def _run_fast_bidask(self) -> None:
        """Fast path: single-symbol bid/ask data, pre-allocated BidAsk reuse."""
        arrays = self.feed.to_arrays()
        n = self.feed.n_bars
        symbol = self.feed.symbols[0]
        ...
        # Pre-allocate one BidAsk object
        ba = BidAsk(timestamp=0, symbol=symbol, bid=0.0, ask=0.0)

        for i in range(n):
            self._event_count += 1

            ba.timestamp = int(ts[i])
            ba.bid = float(b[i])
            ba.ask = float(a[i])
