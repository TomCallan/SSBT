...
def main():
    ...
    strategy = SmaCrossStrategy(fast_period=10, slow_period=30, qty=100.0)
    ...
    print("=" * 40)
    print(f"\nTotal fills: {len(result.fills)}")
    print(f"Total trades (round-trips): {len(result.trades)}")
    ...
    if result.trades:
        trade_data = pl.DataFrame({
            ...
            "qty": [t.qty for t in result.trades],
            "pnl": [t.pnl for t in result.trades],
            "commission": [t.commission for t in result.trades],
        })
        trade_data.write_csv("trade_log.csv")
        print(f"Saved trade_log.csv ({len(result.trades)} trades)")
