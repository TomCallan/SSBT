"""Order matching engine — fills against bar or bid/ask data. Supports all order types, TIF, OCO."""
...
class MatchingEngine:
    ...
    def __init__(
        ...
        # Inline fast path: skip function calls when using default models
        self._use_default = (slippage_fn is default_slippage and commission_fn is default_commission)
        ...
        for i, order in enumerate(self._pending):
            ...
            if fill_price is not None and fill_qty > 0:
                ...
                if self._use_default:
                    ...
                    slipped = fill_price + (slip if order.side == Side.BUY else -slip)
                    commission = slipped * fill_qty * 0.0001
                else:
