...
class Portfolio:
    ...
    def apply_fill(self, fill: Fill) -> None:
        ...
        # Round-trip tracking: match against opposite-side legs FIFO (in-place)
        remaining = fill.qty
        pnl = 0.0
        entry_leg = None
        entry_price = 0.0
        ...
        # Remove closed legs in reverse order (preserves indices)
        for j in sorted(remove_indices, reverse=True):
            legs.pop(j)

        # Any remaining qty opens a new leg (append in-place)
        if remaining > 0:
            legs.append(_OpenFill(
    ...
    def rebalance(
        ...
        """Rebalance portfolio to target weights. Returns fills generated.

        Args:
            target_weights: {symbol: weight} where weights sum to ~1.0.
            prices: {symbol: current_price} for all symbols in target_weights.
            timestamp: ns epoch for the fills.
            commission_bps: commission in basis points.

        Returns:
            List of Fill objects for the rebalance trades.
        """
        total_equity = self.market_value()
        fills: list[Fill] = []
