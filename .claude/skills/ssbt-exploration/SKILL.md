...
description: Interactive skill for executing event studies, strategy backtests, statistical confidence analyses, real-time IPC event streaming, and audit trail verification with SSBT.
---
...
4. **Real-Time IPC Execution Stream**:
...
5. **Integrity & Audit Trail Verification**:
   - Use `ssbt.analytics.audit.AuditLogger` to run anti-lookahead timestamp checks (`verify_event_outcomes`, `verify_backtest`).
   - Export mathematical audit trails (`audit_trail.json` with SHA-256 signatures).

6. **Terminal Displays & Standard Charts**:
...
## Testing Commands
```bash
# Run complete test suite (133 unit & integration tests)
