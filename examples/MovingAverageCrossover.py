...
class MovingAverageCrossover(Strategy):
    """Example strategy using moving average crossover with proper entry and exit signals"""
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        df = data.copy()
        # Skip if indicators are already calculated
        if 'MA_fast' not in df.columns:
            df['MA_fast'] = df['close'].rolling(self.parameters.get('fast_period', 10)).mean()
        if 'MA_slow' not in df.columns:
            df['MA_slow'] = df['close'].rolling(self.parameters.get('slow_period', 30)).mean()
        return df
